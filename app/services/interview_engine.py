...
class InterviewEngine:
    ...
    def start_session(
        ...
        eligible_days = self._build_eligible_days(
            analysis["passed_missions"]
        )

        if not eligible_days:
            raise ValueError(
                "Candidate has no valid passed curriculum days"
            )
        ...
        if preferred_day is not None:
            valid_days = {item.day for item in eligible_days}

            if preferred_day not in valid_days:
                raise ValueError(
                    "Selected curriculum day is not eligible"
                )

            selected_day = preferred_day

        else:
            selected_day = self.choose_next_day(session)

        session.current_day = selected_day
        session.current_topic = self._topic_for_day(
            session,
            selected_day
        )
    ...
    def choose_next_day(
        ...
        unseen_days = [
            ...
            if item.day not in session.asked_days
        ]

        candidates = unseen_days or session.eligible_days

        if not candidates:
            raise ValueError("No eligible curriculum days available")

        question_counts = {
            item.day: session.asked_days.count(item.day)
        ...
        selected = min(
            candidates,
            key=lambda item: (
                question_counts[item.day],
                prior_rank[item.prior],
                item.day,
            ),
        )
    ...
    def record_question(
        ...
        *,
        ...
        if (
            session.transcript
            and session.transcript[-1].answer is None
        ):
            raise ValueError(
                "Answer the current question before asking another one"
            )

        selected_day = (
            day
            if day is not None
            else session.current_day
            if session.current_day is not None
            else self.choose_next_day(session)
        )

        topic = self._topic_for_day(
            session,
            selected_day
        )
    ...
    def record_answer(
        ...
        if (
            ...
            or session.transcript[-1].answer is not None
        ):
            raise ValueError(
                "There is no unanswered interview question"
            )
    ...
    def finish_session(
        ...
        if (
            ...
            raise ValueError(
                "Answer the current question before finishing"
            )
    ...
    def _build_eligible_days(
        self,
        passed_missions: List[dict]
    ) -> List[EligibleDay]:
